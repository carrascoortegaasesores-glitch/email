...
async function sGet(key,fallback=null){
  try{
    ...
    // El servidor ya parsea el JSON, pero por si acaso
...
// ─── Componente: Nueva materia ───────────────────────────────
function NuevaMateria({onAdd,onDelete,materias}){
  const [open,setOpen]=useState(false);
  const [label,setLabel]=useState("");
  const [sub,setSub]=useState("");
  const [detail,setDetail]=useState("");
  const customMaterias=materias.filter(m=>m.custom);

  const handleAdd=()=>{
    if(!label.trim()){alert("Escribe el nombre de la materia.");return;}
    onAdd(label.trim(),sub.trim(),detail.trim());
    setLabel("");setSub("");setDetail("");setOpen(false);
  };
  ...
  return(
    <div style={{marginTop:14}}>
      <button onClick={()=>setOpen(o=>!o)} style={{
        ...
        <span style={{color:C.blue500,fontSize:14}}>＋</span>
        {open?"Cancelar":"Crear nueva materia"}
      </button>

      {open&&(
        <div style={{marginTop:10,padding:14,background:C.blue50,border:`1px solid ${C.blue200}`,borderRadius:10}}>
          <div style={{fontSize:12,fontWeight:700,color:C.blue600,marginBottom:10}}>Nueva materia personalizada</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
            <div>
              <div style={{fontSize:11,color:C.textMid,fontWeight:500,marginBottom:4}}>Nombre *</div>
              <input style={{width:"100%",padding:"8px 10px",border:`1px solid ${C.border2}`,borderRadius:6,fontSize:12,color:C.text,background:"#fff",outline:"none"}}
                placeholder="Ej: IRPF / Declaración anual"
                value={label} onChange={e=>setLabel(e.target.value)}/>
            </div>
            <div>
              <div style={{fontSize:11,color:C.textMid,fontWeight:500,marginBottom:4}}>Subtítulo <span style={{opacity:.6,fontWeight:400}}>(opcional)</span></div>
              <input style={{width:"100%",padding:"8px 10px",border:`1px solid ${C.border2}`,borderRadius:6,fontSize:12,color:C.text,background:"#fff",outline:"none"}}
                placeholder="Ej: Modelo 100"
                value={sub} onChange={e=>setSub(e.target.value)}/>
            </div>
          </div>
          <div style={{marginBottom:10}}>
            <div style={{fontSize:11,color:C.textMid,fontWeight:500,marginBottom:4}}>Descripción para la IA <span style={{opacity:.6,fontWeight:400}}>(opcional)</span></div>
            <input style={{width:"100%",padding:"8px 10px",border:`1px solid ${C.border2}`,borderRadius:6,fontSize:12,color:C.text,background:"#fff",outline:"none"}}
              placeholder="Ej: Dudas sobre la declaración anual de IRPF, modelo 100, deducciones..."
              value={detail} onChange={e=>setDetail(e.target.value)}/>
          </div>
          <button onClick={handleAdd} style={{background:`linear-gradient(135deg,${C.blue600},${C.blue800})`,color:"#fff",border:"none",borderRadius:7,padding:"9px 18px",fontSize:12,fontWeight:700,cursor:"pointer",boxShadow:"0 2px 6px rgba(30,91,175,0.25)"}}>
            Crear materia
          </button>
        </div>
      )}
...
// ─── Vista: Historial ─────────────────────────────────────────
function VistaHistorial({historial,empleados,empleadoActual,materias}){
...
function VistaEjemplos({ejemplosPorMateria,onDelete,onRename,empleados,materias}){
  const [matActiva,setMatActiva]=useState(null);
  const [editando,setEditando]=useState(null); // {matId, idx}
                        ...
                        <div key={i} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:8,padding:12}}>
                          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                            ...
                            <div style={{display:"flex",gap:6}}>
                              {editando?.matId===m.id&&editando?.idx===i?(
                                <div style={{display:"flex",gap:4,alignItems:"center"}}>
                                  <input value={tituloEdit} onChange={e=>setTituloEdit(e.target.value)}
                                    onKeyDown={e=>{if(e.key==="Enter"){onRename(m.id,i,tituloEdit);setEditando(null);}if(e.key==="Escape")setEditando(null);}}
                                    autoFocus style={{padding:"2px 8px",border:`1px solid ${C.blue400}`,borderRadius:5,fontSize:11,outline:"none",width:140}}
                                    placeholder="Título del ejemplo"/>
                                  <button onClick={()=>{onRename(m.id,i,tituloEdit);setEditando(null);}} style={{background:C.blue600,border:"none",color:"#fff",cursor:"pointer",fontSize:10,padding:"3px 8px",borderRadius:5}}>✓</button>
                                  <button onClick={()=>setEditando(null)} style={{background:"none",border:`1px solid ${C.border}`,color:C.textLight,cursor:"pointer",fontSize:10,padding:"3px 8px",borderRadius:5}}>✕</button>
                                </div>
                              ):(
                                <button onClick={()=>{setEditando({matId:m.id,idx:i});setTituloEdit(ej.titulo||"");}} style={{background:"none",border:`1px solid ${C.border}`,cursor:"pointer",fontSize:11,color:C.textLight,padding:"2px 8px",borderRadius:5}}>
                                  {ej.titulo?"✏️ "+ej.titulo:"+ Título"}
                                </button>
                              )}
...
export default function App(){
  const [materias,setMaterias]=useState(BASE_MATERIAS);
...
- Si falta información para responder con precisión, indícalo.`;  };
  ...
  return(
    ...
    return sys;eState, useRef, useCallback, useEffect } from "react";
