...
function VistaEjemplos({ejemplosPorMateria,onDelete,empleados,materias}){
...
- Texto listo para enviar.`;
  ...
  const generate=async()=>{
    ...
    try{
      const res=await fetch("/api/claude",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-5-20250929",max_tokens:1000,system:buildSystem(),messages:[{role:"user",content:buildPrompt()}]})});
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'Helvetica Neue',Arial,sans-serif",color:C.text}}>
      ...
      <div style={{maxWidth:860,margin:"0 auto",padding:"32px 32px 100px"}}>
        ...
        {/* ── TAB: EJEMPLOS ── */}
        {tab==="ejemplos"&&<VistaEjemplos ejemplosPorMateria={ejemplosPorMateria} onDelete={deleteEjemplo} empleados={empleados} materias={materias}/>}
